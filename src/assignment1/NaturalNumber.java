...
public class NaturalNumber  {
	...
	public NaturalNumber plus( NaturalNumber  second) throws Exception{
		...
		for (int k = 0; k < firstClone.coefficients.size(); k++) {
			...
			if (sum.coefficients.get(k)>= sum.getBase()) {
				...
				sum.coefficients.set(k, sum.coefficients.get(k) % sum.base);
			}
			else{
	...
	public NaturalNumber times( NaturalNumber multiplicand) throws Exception{
		...
		for(int k = 0; k < this.coefficients.size(); k++){
			...
			for(int j = 0; j < multiplicand.coefficients.size(); j++){
				...
				temp.coefficients.add(this.coefficients.get(k) * multiplicand.coefficients.get(j));
	...
	private NaturalNumber timesSingleDigit(NaturalNumber unused) {
		
		//this method is not used. Implemented everything in product().
	...
	public NaturalNumber  minus(NaturalNumber second) throws Exception{
		...
		for(int diff = first.coefficients.size() - second.coefficients.size(); diff > 0; diff--){
			
			difference.coefficients.add(first.coefficients.get(first.coefficients.size() - diff));
		...
		for(int k = second.coefficients.size() - 1; k >= 0 ; k--){
			...
			if(first.coefficients.get(k) < second.coefficients.get(k)){
				...
				while(difference.coefficients.get(extraBorrows) == 0){
					...
					extraBorrows = extraBorrows + 1;
				}
				
				for(int j = extraBorrows - 1; j >= 0 ; j--){
					
					difference.coefficients.set(j, difference.getBase() - 1);
				}
				
				difference.coefficients.set(extraBorrows, difference.coefficients.get(extraBorrows) - 1);

				first.coefficients.set(k, first.coefficients.get(k) + first.getBase());	
			}
			
			difference.coefficients.addFirst(first.coefficients.get(k) - second.coefficients.get(k));	
		}
